...
RSpec.describe Baron::Rules do
  ...
  describe '#private_companies' do
    ...
    it 'initializes them as private companies' do
      expect(subject.all? do |c|
        c.kind_of? Baron::Company::PrivateCompany
      end).to be true
    end

    it 'loads their abbreviations' do
      expect(subject.map(&:abbreviation)).to contain_exactly(
        'BHC',
        'YHC',
        'CM&H',
        'RP&SC',
    ...
    it 'loads their face values' do
      expect(subject.map(&:face_value)).to contain_exactly(
        Baron::Money.new(30),
        Baron::Money.new(50),
        Baron::Money.new(90),
        Baron::Money.new(130),
    ...
    it 'loads their revenues' do
      expect(subject.map(&:revenue)).to contain_exactly(
        Baron::Money.new(5),
        Baron::Money.new(10),
        Baron::Money.new(20),
        Baron::Money.new(30),
  ...
  describe '#major_companies' do
    ...
    it 'initializes them as major companies' do
      expect(subject.all? do |c|
        c.kind_of? Baron::Company::MajorCompany
      end).to be true
    end
